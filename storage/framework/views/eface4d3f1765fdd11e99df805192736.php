<?php $__env->startSection('title', 'จัดการสัญญาเช่า'); ?>

<?php $__env->startSection('page-title', 'จัดการสัญญาเช่า'); ?>

<?php $__env->startSection('content'); ?>
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="bi bi-file-earmark-text me-2"></i>รายการสัญญาเช่า</h4>
    <div class="d-flex gap-2">
        <a href="<?php echo e(route('contracts.expiring')); ?>" class="btn btn-outline-warning">
            <i class="bi bi-exclamation-triangle me-1"></i>สัญญาหมดอายุเร็ว
        </a>
        <a href="<?php echo e(route('contracts.export')); ?>" class="btn btn-success">
            <i class="bi bi-download me-1"></i><?php echo e(__("ui.export")); ?>

        </a>
        <a href="<?php echo e(route('contracts.create')); ?>" class="btn btn-primary-custom">
            <i class="bi bi-plus-lg me-1"></i>เพิ่มสัญญา
        </a>
    </div>
</div>

<!-- Search and Filter -->
<div class="card mb-4">
    <div class="card-body">
        <form method="GET" action="<?php echo e(route('contracts.index')); ?>" class="row g-3">
            <div class="col-md-4">
                <input type="text" name="search" class="form-control" placeholder="ค้นหาเลขที่สัญญา, ชื่อผู้เช่า, ห้องพัก..." value="<?php echo e(request('search')); ?>">
            </div>
            <div class="col-md-3">
                <select name="status" class="form-select">
                    <option value="">ทุกสถานะ</option>
                    <option value="draft" <?php echo e(request('status') == 'draft' ? 'selected' : ''); ?>>ร่างสัญญา</option>
                    <option value="pending" <?php echo e(request('status') == 'pending' ? 'selected' : ''); ?>>รออนุมัติ</option>
                    <option value="active" <?php echo e(request('status') == 'active' ? 'selected' : ''); ?>>ใช้งาน</option>
                    <option value="completed" <?php echo e(request('status') == 'completed' ? 'selected' : ''); ?>>เสร็จสิ้น</option>
                    <option value="cancelled" <?php echo e(request('status') == 'cancelled' ? 'selected' : ''); ?>>ยกเลิก</option>
                </select>
            </div>
            <div class="col-md-2">
                <button type="submit" class="btn btn-primary w-100">
                    <i class="bi bi-search me-1"></i><?php echo e(__("ui.search")); ?>

                </button>
            </div>
        </form>
    </div>
</div>

<!-- Contracts Table -->
<div class="table-card">
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            <thead>
                <tr>
                    <th>เลขที่สัญญา</th>
                    <th>ห้องพัก</th>
                    <th>ผู้เช่า</th>
                    <th>วันเริ่มต้น</th>
                    <th>วันสิ้นสุด</th>
                    <th>ค่าเช่า/เดือน</th>
                    <th>สถานะ</th>
                    <th>การกระทำ</th>
                </tr>
            </thead>
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $contracts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $contract): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr>
                        <td><strong><?php echo e($contract->contract_number); ?></strong></td>
                        <td>
                            <?php if($contract->room): ?>
                                <span class="badge bg-primary"><?php echo e($contract->room->room_number); ?></span>
                            <?php else: ?>
                                <span class="text-muted">-</span>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if($contract->guest): ?>
                                <?php echo e($contract->guest->first_name); ?> <?php echo e($contract->guest->last_name); ?>

                            <?php else: ?>
                                <span class="text-muted">-</span>
                            <?php endif; ?>
                        </td>
                        <td><?php echo e(optional($contract->start_date)->format('d/m/Y')); ?></td>
                        <td><?php echo e(optional($contract->end_date)->format('d/m/Y')); ?></td>
                        <td>฿<?php echo e(number_format($contract->monthly_rent, 2)); ?></td>
                        <td>
                            <?php
                                $statusClasses = [
                                    'draft' => 'bg-secondary',
                                    'pending' => 'bg-warning text-dark',
                                    'active' => 'bg-success',
                                    'completed' => 'bg-info',
                                    'cancelled' => 'bg-danger',
                                ];
                            ?>
                            <span class="badge <?php echo e($statusClasses[$contract->status] ?? 'bg-secondary'); ?>">
                                <?php echo e(enum_bi('contract_status', $contract->status, ucfirst($contract->status))); ?>

                            </span>
                        </td>
                        <td>
                            <div class="d-flex gap-2">
                                <a href="<?php echo e(route('contracts.show', $contract)); ?>" class="btn btn-sm btn-outline-info">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="<?php echo e(route('contracts.edit', $contract)); ?>" class="btn btn-sm btn-outline-warning">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <a href="<?php echo e(route('contracts.pdf', $contract)); ?>" class="btn btn-sm btn-outline-danger" target="_blank">
                                    <i class="bi bi-file-pdf"></i>
                                </a>
                            </div>
                        </td>
                    </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="8" class="text-center py-4">
                            <i class="bi bi-inbox fs-1 text-muted"></i>
                            <p class="text-muted mt-2">ไม่มีข้อมูลสัญญาเช่า</p>
                            <a href="<?php echo e(route('contracts.create')); ?>" class="btn btn-primary-custom mt-2">เพิ่มสัญญาเช่า</a>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
</div>

<!-- Pagination -->
<?php if($contracts->hasPages()): ?>
    <div class="d-flex justify-content-center mt-4">
        <?php echo e($contracts->links('pagination::bootstrap-5')); ?>

    </div>
<?php endif; ?>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Rm1\resources\views/contracts/index.blade.php ENDPATH**/ ?>