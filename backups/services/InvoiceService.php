...
/**
 * InvoiceService - COMPLETE & ENHANCED
 * 
...
class InvoiceService
{
    ...
    /**
     * ✅ NEW: Prepare data for invoice creation
     *
     * Handles:
     * - Generate invoice number if not provided
     * - Calculate total from amount + tax
    ...
    public function prepareForCreate(array $data): array
    {
        ...
        $expectedTotal = $this->calculateTotal(
            (float)$data['amount'],
            (float)$data['tax']
        );
 
        if (abs((float)$data['total'] - $expectedTotal) > 0.01) {
            \Log::warning('InvoiceService prepareForCreate total mismatch', [
                'provided' => $data['total'],
    ...
    /**
     * ✅ NEW: Prepare data for invoice update
     *
     * Handles:
     * - Recalculate total from amount + tax
     * - Maintain invoice number (don't change)
     */
    public function prepareForUpdate(array $data): array
    {
        ...
        // Never change invoice number on update
    ...
    /**
     * ✅ NEW: Format invoices for Excel export
     */
    public function formatForExport(Collection $invoices): array
    {
        $rows = [
            ['Invoice Number', 'Booking ID', 'Guest Name', 'Room', 'Amount', 'Tax', 'Total', 'Issue Date', 'Due Date', 'Status', 'Notes'],
        ];
        ...
        foreach ($invoices as $invoice) {
            $rows[] = [
                ...
                $invoice->booking_id ?? '-',
                trim(
                    ($invoice->booking?->guest?->first_name ?? '') . ' ' .
                    ($invoice->booking?->guest?->last_name ?? '')
                ) ?: '-',
