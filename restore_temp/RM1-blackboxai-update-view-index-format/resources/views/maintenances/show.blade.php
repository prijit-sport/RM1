...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="value"><strong>ห้อง {{ $maintenance->room->room_number ?? '-' }}</strong></div>
    </div>

    <div class="row">
        <div class="label">ประเภทปัญหา:</div>
        <div class="value">
            <span class="fw-bold">{{ $maintenance->maintenance_type }}</span>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="value">
            <span class="badge status-{{ $maintenance->status }}">
                @switch($maintenance->status)
                    @case('pending') รอดำเนินการ @break
                    @case('in_progress') กำลังดำเนินการ @break
    ...
    <div class="row">
        ...
        <div class="value"><i class="fas fa-user-gear me-1"></i> {{ $maintenance->assigned_to ?? 'ยังไม่ระบุ' }}</div>
    </div>

    <div class="row">
        <div class="label">ค่าใช้จ่าย:</div>
        <div class="value" style="color: #ef4444; font-weight: 700;">
            {{ $maintenance->cost ? number_format($maintenance->cost, 2) . ' บาท' : '0.00 บาท' }}
        </div>
    </div>

    <div class="row">
        <div class="label">วันที่แจ้งซ่อม:</div>
        <div class="value">{{ optional($maintenance->request_date)->format('d/m/Y') ?? '-' }}</div>
    </div>

    <div class="row">
        <div class="label">รายละเอียดปัญหา:</div>
        <div class="value">{{ $maintenance->description ?? '-' }}</div>
    </div>

    <div class="row">
        <div class="label">หมายเหตุเพิ่มเติม:</div>
        <div class="value">{{ $maintenance->notes ?? '-' }}</div>
    </div>
    ...
    <div class="btn-group">
        ...
        <a href="{{ route('maintenances.edit', $maintenance->id) }}" class="btn btn-edit">
