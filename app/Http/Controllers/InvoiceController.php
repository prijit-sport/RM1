...
class InvoiceController extends Controller
{
    ...
    public function bulkStore(Request $request): RedirectResponse
    {
        ...
        try {
            $result = $this->invoiceService->bulkCreateFromBookings(
                validated: $validated,
        ...
        $rolloverWarnings = $result['rollover_warnings'] ?? [];
        ...
        // ✅ FIX: แจ้งผู้ใช้ว่าห้องไหนถูกข้ามไปบ้าง (เช่น ไม่มีข้อมูลมิเตอร์) และห้องไหนที่
        // เลขมิเตอร์ย้อนกลับโดยไม่ได้ติ๊ก "มิเตอร์เปลี่ยนใหม่" — รวมเป็นข้อความเดียวเพราะ
        // session flash เก็บได้แค่ 1 ค่าต่อ key ถ้าเรียก ->with('warning', ...) ซ้ำจะทับกัน
        $warningLines = [];
        if (! empty($skipped)) {
            $roomNumbers = collect($skipped)->pluck('room_number')->implode(', ');
            $warningLines[] = 'ข้ามห้องที่ไม่มีข้อมูลมิเตอร์ของรอบบิลนี้: '.$roomNumbers;
        }
        if (! empty($rolloverWarnings)) {
            $roomList = collect($rolloverWarnings)->pluck('room_number')->implode(', ');
            $warningLines[] = 'พบเลขมิเตอร์ย้อนกลับ (อาจมิเตอร์เปลี่ยนใหม่หรือกรอกผิด) ที่ห้อง: '.$roomList
                .' — ระบบคิดหน่วยการใช้เป็น 0 ไว้ก่อน กรุณาตรวจสอบและแก้ไข reading พร้อมติ๊ก "มิเตอร์เปลี่ยนใหม่" ถ้าถูกต้อง';
        }
        if (! empty($warningLines)) {
            $redirect->with('warning', implode(' | ', $warningLines));
        }
