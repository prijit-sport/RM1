...
class BookingController extends Controller
{
    ...
    public function confirm(Request $request, Booking $booking)
    {
        ...
        $validated = $request->validate([
            ...
            'electric_rate.required' => 'กรุณากรอกอัตราค่าไฟฟ้า',
            'electric_rate.numeric'  => 'อัตราค่าไฟฟ้าต้องเป็นตัวเลข',
            'water_rate.required'    => 'กรุณากรอกอัตราค่าน้ำ',
            'water_rate.numeric'     => 'อัตราค่าน้ำต้องเป็นตัวเลข',
        ]);
 
        $validated['tax_rate'] = $validated['tax_rate'] ?? 0;
 
        $this->bookingService->confirm($booking, $validated);
 
        return redirect()
            ->route('bookings.show', $booking)
            ->with('success', 'ยืนยันการจองสำเร็จ สร้างมิเตอร์น้ำ/ไฟเรียบร้อยแล้ว');
    }
