...
class ReportController extends Controller
{
    ...
    private function getMetersData(): array
    {
        ...
        try {
            // สมมุติว่ามีตาราง meter_readings (electric_usage, water_usage, reading_date, room_id)
            $current_month_electric = (float) DB::table('meter_readings')
                ->whereMonth('reading_date', now()->month)
                ->whereYear('reading_date', now()->year)
                ->sum('electric_usage');

            $current_month_water = (float) DB::table('meter_readings')
                ->whereMonth('reading_date', now()->month)
                ->whereYear('reading_date', now()->year)
                ->sum('water_usage');

            $top_electric_rooms = DB::table('meter_readings')
                ->join('rooms', 'meter_readings.room_id', '=', 'rooms.id')
                ->whereMonth('reading_date', now()->month)
                ->selectRaw('rooms.room_number, SUM(meter_readings.electric_usage) as total')
        ...
        return compact('current_month_electric', 'current_month_water', 'top_electric_rooms');
    }
