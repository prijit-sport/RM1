                        ...
                        <form action="{{ route('meters.update', $meter) }}" method="POST">
                                    ...
                                    <select id="room_id" name="room_id"
                                        ...
                                        @foreach ($rooms ?? [] as $room)
                                            ...
                                            <option value="{{ $room->id }}"
                                                {{ old('room_id', $meter->room_id) == $room->id ? 'selected' : '' }}>
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-md-6">
                                    ...
                                    <select id="type" name="type"
                                        ...
                                        <option value="electric"
                                            {{ old('type', $meter->type) == 'electric' ? 'selected' : '' }}>ไฟฟ้า</option>
                                        <option value="water" {{ old('type', $meter->type) == 'water' ? 'selected' : '' }}>
                                            น้ำ</option>
                                    </select>
                                    @error('type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input id="meter_number" type="text" name="meter_number"
                                        class="form-control @error('meter_number') is-invalid @enderror"
                                        value="{{ old('meter_number', $meter->meter_number) }}" required>
                                    @error('meter_number')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row g-3 mb-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input id="installed_at" type="date" name="installed_at"
                                        class="form-control @error('installed_at') is-invalid @enderror"
                                        value="{{ old('installed_at', optional($meter->installed_at)->format('Y-m-d')) }}">
                                    @error('installed_at')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-md-6">
                                    ...
                                    <input id="rate_per_unit" type="number" step="0.01" name="rate_per_unit"
                                        ...
                                        value="{{ old('rate_per_unit', $meter->rate_per_unit) }}" required>
                                    @error('rate_per_unit')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input id="tax_rate" type="number" step="0.01" name="tax_rate"
                                        ...
                                        value="{{ old('tax_rate', $meter->tax_rate) }}" required>
                                    @error('tax_rate')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-md-6">
                                    ...
                                    <select id="is_active" name="is_active"
                                        ...
                                        <option value="1"
                                            {{ old('is_active', $meter->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>
                                            ใช้งาน</option>
                                        <option value="0"
                                            {{ old('is_active', $meter->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>
                                            ระงับใช้งาน</option>
                                    </select>
                                    @error('is_active')
                                        <div class="invalid-feedback">{{ $message }}</div>
