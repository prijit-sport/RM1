...
<body>
    ...
    <script nonce="{{ $cspNonce ?? '' }}">
        ...
        // ✅ FIX (CSP nonce-based): แทนที่ onclick="return confirm(...)" /
        // onsubmit="return confirm(...)" ที่กระจายอยู่หลายหน้า ด้วย delegated
        // listener ตัวเดียวตรงนี้ — ใส่ attribute data-confirm="ข้อความ" บน
        // <form> หรือปุ่ม/ลิงก์ แทน inline event handler (ซึ่ง CSP nonce
        // ครอบคลุมไม่ถึง ต้องแก้ที่ต้นตอแบบนี้)
        ...
        // ✅ FIX (CSP nonce-based): แทนที่ onchange="this.form.submit()" ด้วย
        // data-auto-submit attribute + delegated listener ตัวนี้
        document.addEventListener('change', function(e) {
            if (e.target.hasAttribute && e.target.hasAttribute('data-auto-submit')) {
