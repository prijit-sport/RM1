            ...
            <form action="{{ route('bookings.update', $booking) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        <option value="pending" {{ old('status', $booking->status) == 'pending' ? 'selected' : '' }}>รอการยืนยัน</option>
                        <option value="confirmed" {{ old('status', $booking->status) == 'confirmed' ? 'selected' : '' }}>ยืนยันแล้ว</option>
                        <option value="checked_in" {{ old('status', $booking->status) == 'checked_in' ? 'selected' : '' }}>เช็คอินแล้ว</option>
                        <option value="checked_out" {{ old('status', $booking->status) == 'checked_out' ? 'selected' : '' }}>เช็คเอาท์แล้ว</option>
                        <option value="cancelled" {{ old('status', $booking->status) == 'cancelled' ? 'selected' : '' }}>ยกเลิก</option>
                    </select>
                    @error('status')
                        <div class="error">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea id="notes" name="notes" placeholder="เขียนหมายเหตุ..." rows="4">{{ old('notes', $booking->notes) }}</textarea>
                    @error('notes')
                        <div class="error">{{ $message }}</div>
                    @enderror
                </div>
