                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="room_type_display"
                                class="form-control bg-light" placeholder="—" readonly>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                ...
                                <input type="number" name="electric_meter_start"
                                    ...
                                    placeholder="0" min="0"
                                    value="{{ old('electric_meter_start', 0) }}">
...
<script>
    ...
    const typeLabel = { 'fan': '🌀 พัดลม', 'air': '❄️ แอร์' };
    ...
    zoneSelect.addEventListener('change', function () {
        ...
        if (!zone) { roomSelect.disabled = true; return; }
 
        // ✅ แก้: ไม่กรอง status แล้ว — แสดงทุกห้องในโซน
        const filtered = allRooms.filter(r => r.zone === zone);
        ...
        filtered.forEach(room => {
            ...
            opt.textContent   = `${room.room_number} — ${typeLabel[room.room_type] ?? room.room_type} (${Number(room.price_per_month).toLocaleString()} ฿/เดือน)`;
            roomSelect.appendChild(opt);
        });
 
        roomSelect.disabled = filtered.length === 0;
        if (filtered.length === 0) {
            roomSelect.innerHTML = '<option value="">ไม่มีห้องในโซนนี้</option>';
        }
    });
