    ...
    <div class="container">
        ...
        <form method="POST" action="{{ route('invoices.update', $invoice->id) }}">
            ...
            <div class="form-group">
                ...
                <input type="number" id="amount" name="amount" value="{{ old('amount', $invoice->amount) }}" step="0.01" required>
                @error('amount')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="tax" name="tax" value="{{ old('tax', $invoice->tax) }}" step="0.01" required>
                @error('tax')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="total" name="total" value="{{ old('total', $invoice->total) }}" step="0.01" required>
                @error('total')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" id="issue_date" name="issue_date" value="{{ old('issue_date', $invoice->issue_date->format('Y-m-d')) }}" required>
                @error('issue_date')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="btn-group">
                ...
                <a href="{{ route('invoices.show', $invoice->id) }}" class="btn btn-cancel">❌ ยกเลิก</a>
            </div>
        </form>
    </div>
</body>
